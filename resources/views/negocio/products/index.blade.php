            ...
            @foreach ($products as $product)
                <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                    <a href="{{ route('negocio.products.edit', $product) }}" class="flex justify-center">
                        <img class="rounded-t-lg h-[250px]" src="{{ asset('storage/'.$product->imagen) }}" alt="{{ $product->nombre }}" />
                    </a>
                    <div class="p-5">
                        <div>
                            <h5 class="mb-2 text-xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $product->nombre }}</h5>
                        ...
                        <div class="flex gap-2">
                            ...
                            {{ Aire::open()->route('negocio.products.destroy', $product) }}
                                <button type="submit" class="btn-secondary" onclick="return confirm('¿Seguro que desea eliminar este producto?')">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" class="w-5 h-5">
